      ...
      {steps.map((label, i) => (
        <div key={i} className="flex items-center">
          <div className="flex flex-col items-center">
            ...
            <span className="font-sans text-[10px] tracking-[0.3em] uppercase mt-2"
              style={{ color: i + 1 === step ? "#2C2C2C" : "rgba(44,44,44,0.35)" }}>
              {label}
            </span>
          </div>
          {i < steps.length - 1 && (
            <div className="w-16 md:w-24 h-px mx-3 mb-5 transition-colors duration-300"
              style={{ backgroundColor: i + 1 < step ? "#6B7C5C" : "rgba(44,44,44,0.12)" }} />
          )}
        </div>
      ))}
    </div>
...
function BtnPrimary({ label, onClick, type = "button", disabled = false }: {
  ...
  return (
    <button
      ...
      onClick={onClick}
      disabled={disabled}
      className="group relative flex-1 border border-[#2C2C2C] text-[#2C2C2C] font-sans text-[11px] tracking-[0.4em] uppercase py-5 overflow-hidden hover:text-[#F5F0E8] transition-colors duration-500 disabled:opacity-40 disabled:cursor-not-allowed"
    >
...
function BtnSecondary({ label, onClick }: { label: string; onClick: () => void }) {
  return (
    <button
      type="button"
      onClick={onClick}
      className="flex-1 border border-[#2C2C2C]/20 text-[#2C2C2C]/50 font-sans text-[11px] tracking-[0.4em] uppercase py-5 hover:border-[#2C2C2C]/40 hover:text-[#2C2C2C]/70 transition-colors duration-300"
    >
      {label}
    </button>
  );
...
function Step1({ experience, date, guests, onNext, onChangeDetails }: any) {
  ...
  return (
    <div>
      ...
      <h1 className="font-serif text-[clamp(2rem,4vw,3rem)] text-[#2C2C2C] leading-tight mb-12">Your booking</h1>

      <div className="border border-[#2C2C2C]/15 p-8 mb-8" style={{ backgroundColor: "#F8F4EE" }}>
        <h2 className="font-serif text-xl text-[#2C2C2C] mb-8">{experience.title}</h2>
        ...
        <div className="border-t border-[#2C2C2C]/10 pt-6">
          <div className="flex justify-between items-center mb-1">
            ...
            <p className="font-serif text-3xl text-[#2C2C2C]">{formatPrice(total)}</p>
          </div>
      ...
      <button
        ...
        className="font-sans text-[11px] tracking-[0.4em] uppercase text-[#2C2C2C]/35 underline underline-offset-4 mb-10 block hover:text-[#2C2C2C] transition-colors"
      >
        Change details
      </button>
...
function Step2({ onNext, onBack, customerData, setCustomerData }: any) {
  ...
  const inputClass = "w-full border border-[#2C2C2C]/15 px-4 py-3 font-sans text-sm text-[#2C2C2C] focus:outline-none focus:border-[#6B7C5C] transition-colors";
  const inputStyle = { backgroundColor: "#F8F4EE" };

  return (
    <div>
      <p className="font-sans text-[12px] tracking-[0.5em] uppercase text-[#6B7C5C] mb-4">Step 2</p>
      <h1 className="font-serif text-[clamp(2rem,4vw,3rem)] text-[#2C2C2C] leading-tight mb-2">Almost there.</h1>
      <p className="font-sans text-xl text-[#2C2C2C]/55 leading-relaxed mb-12" style={{ fontWeight: 300 }}>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div>
          ...
          <textarea value={customerData.notes} onChange={e => setCustomerData({ ...customerData, notes: e.target.value })} rows={3} className={`${inputClass} resize-none`} style={inputStyle} placeholder="Dietary requirements, special occasions, anything your host should know..." />
        </div>
...
function PaymentForm({ experience, date, guests, customerData, onBack, onSuccess }: any) {
  ...
  return (
    <form onSubmit={handleSubmit}>
      ...
      <h1 className="font-serif text-[clamp(2rem,4vw,3rem)] text-[#2C2C2C] leading-tight mb-12">Secure your experience.</h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-10">
        ...
        <div>
          ...
          <div className="flex items-center gap-2 mt-4">
            <svg className="w-4 h-4 text-[#6B7C5C] flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
              <path strokeLinecap="round" strokeLinejoin="round" d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z" />
            </svg>
            <p className="font-sans text-[11px] text-[#2C2C2C]/40" style={{ fontWeight: 300 }}>Secured by Stripe. Your card data never touches our servers.</p>
          </div>
        </div>

        {/* Recap */}
        <div className="border border-[#2C2C2C]/15 p-6" style={{ backgroundColor: "#F8F4EE" }}>
          <p className="font-sans text-[11px] tracking-[0.4em] uppercase text-[#2C2C2C]/40 mb-4">Your booking</p>
          <p className="font-serif text-xl text-[#2C2C2C] mb-6">{experience.title}</p>
          <div className="space-y-3 mb-6">
            {[
              ...
              <div key={item.label} className="flex justify-between">
                ...
                <span className="font-sans text-[11px] text-[#2C2C2C]">{item.value}</span>
              </div>
            ))}
          </div>
          <div className="border-t border-[#2C2C2C]/10 pt-4 flex justify-between items-center">
            ...
            <span className="font-serif text-2xl text-[#2C2C2C]">{formatPrice(total)}</span>
          </div>
...
function Confirmation({ experience, date, customerData }: any) {
  return (
    <div className="min-h-screen pt-28 pb-20" style={{ backgroundColor: "#F5F0E8" }}>
      <div className="max-w-2xl mx-auto px-8">
        ...
        <h1 className="font-serif text-[clamp(2.5rem,6vw,5rem)] text-[#2C2C2C] leading-[1.0] mb-8">
          Your Provence<br /><em>is confirmed.</em>
        </h1>
        <p className="font-sans text-xl text-[#2C2C2C]/55 leading-relaxed mb-3" style={{ fontWeight: 300 }}>
        ...
        <p className="font-sans text-xl text-[#2C2C2C]/55 leading-relaxed mb-16" style={{ fontWeight: 300 }}>
          A confirmation email has been sent to {customerData.email}.
        </p>
        <div className="border border-[#2C2C2C]/15 p-8 mb-16" style={{ backgroundColor: "#F8F4EE" }}>
          {[
            ...
            <div key={item.time} className="flex gap-6 py-4 border-b border-[#2C2C2C]/08 last:border-0">
              <span className="font-sans text-[11px] tracking-[0.3em] uppercase text-[#6B7C5C] w-28 flex-shrink-0 pt-0.5">{item.time}</span>
              <span className="font-sans text-base text-[#2C2C2C]" style={{ fontWeight: 300 }}>{item.text}</span>
            </div>
          ))}
        </div>
        <p className="font-serif text-xl italic text-[#2C2C2C]/40">
...
function BookContent() {
  ...
  return (
    <>
      <Navbar />
      <main className="min-h-screen pt-28 pb-20" style={{ backgroundColor: "#F5F0E8" }}>
        <div className="max-w-2xl mx-auto px-8">
          <div className="flex items-center gap-4 mb-16">
            <span className="block w-8 h-px bg-[#6B7C5C]" />
